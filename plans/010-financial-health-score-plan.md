# Implementation Plan: Financial Health Score Tool

This plan details the implementation steps to add the `financial_health_score` MCP tool to the Finance Intelligence MCP server, creating a new module `health.py` and calling it in `main.py`.

## User Review Required

- **No Breaking Changes**: The `financial_health_score` tool is a completely new feature.
- **Deterministic Rules**: The tool calculates mathematical indicators based on target month and previous months' data. If database history is sparse (e.g., first month using the app), it gracefully defaults to neutral values (e.g. 7 out of 10) with informative feedback.
...
#### [NEW] [health.py](file:///c:/Users/Admin/Desktop/Finance%20Intelligence%20MCP/health.py)
We will create `health.py` containing the scoring math and database queries:
- Imports: `os`, `datetime` (`date`, `timedelta`), and `statistics`.
- Classify standard categories into:
  - **Essentials**: `['food', 'transport', 'housing', 'utilities', 'health', 'family_kids', 'home', 'taxes', 'finance_fees']`
  - **Discretionaries**: `['entertainment', 'shopping', 'travel', 'subscriptions', 'personal_care', 'gifts_donations', 'misc', 'business', 'investments']`
- Query database:
  - Fetches active budgets and their matching expense sums in the reference month.
  - Fetches category distributions for the reference month.
  - Fetches maximum transaction amount in the reference month.
  - Fetches total monthly expenses for target month (M), previous month (M-1), and month before (M-2) to evaluate stability and MoM trend.
- Calculate deterministic sub-scores (each out of 10):
  - **Budget Adherence**: Checks frequency and average overshoot of exceeded budgets.
  - **Expense Stability**: CV (Coefficient of Variation) over M, M-1, and M-2.
  - **Savings Capacity**: Total budget limit vs. actual spending.
  - **Category Balance**: Discretionary ratio vs. Wants budget target (wants <= 30%).
  - **Large Expense Ratio**: Max single transaction vs. total monthly spend.
  - **Spending Trend**: MoM comparison of total spend.
- Normalize sum to 100 and evaluate grade (Excellent, Good, Fair, Poor).
...
async def financial_health_score_impl(conn, user_id: int, reference_month: str = None) -> dict:
    ...
    if total_active_budgets == 0:
        ...
        score = 10
        deduct_freq = (exceeded_count / total_active_budgets) * 5
        score -= deduct_freq
    ...
    if overall_budget_total == 0:
        ...
        if savings_ratio >= 0.20:
            ...
            score = 0
            msg = f"Warning: Spent exceeded budget totals by {round(abs(savings_ratio) * 100, 1)}%."
